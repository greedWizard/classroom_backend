...
class RoomPostService(AuthorMixin, CRUDService):
    ...
    async def fetch(
        ...
        **filters,
        ...
        if search:
            if not await self._repository.search_fetch(_ordering, join, limit=limit, offset=offset, search=search, **filters):
                return None, {'error': _('Not Found!')}
            return await self._repository.search_fetch(_ordering, join, limit=limit, offset=offset, search=search, **filters), None
