...
@pytest.mark.asyncio
async def test_participations_search_list(
    app: FastAPI,
    client: FastAPITestClient,
    participation_repository: ParticipationRepository,
    room_repository: RoomRepository,
    ...
    client.authorize(user)
    url = app.url_path_for('get_participations')
    response = client.get(url, params={'room_id': room.id, 'search': room.name})
    json_data = response.json()
    ...
    response = client.get(url, params={'room_id': room.id, 'search': 'Слава вернись'})
    json_data = response.json()
